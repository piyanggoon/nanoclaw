...
Each group has its own filesystem, memory, and IPC namespace. Per-group session isolation prevents cross-group information disclosure.
...
**Core components:**
- **Claude Agent SDK** as the core agent (runs as subprocess)
...
### Agent Execution
- Agents run as Node.js subprocesses via the Claude Agent SDK
- Each invocation spawns a subprocess with configured environment variables
- Per-group working directories and IPC namespaces provide isolation
...
- Custom `nanoclaw` MCP server (in agent subprocess) provides scheduling tools
